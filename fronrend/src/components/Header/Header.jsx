...
const items = [
    {
        ...
        label: (
            ...
            <img src={logo} alt="Лого" style={{ height: '3.1vw' }} />
          </Link>
        )
    },
...
export default function HeaderFunc(props) {
  ...
  return (
    ...
    <Menu
      theme="dark"
      mode="horizontal"
      style={{flex: 1, display: 'flex', justifyContent: 'space-between', width: '100%', fontSize: '1.3vw', marginRight: '15%', fontFamily: 'pragmatica', fontWeight: 'lighter'}}
      
    >
      {items.map(item => (
        item.subItems ? (
          <Menu.SubMenu key={item.key} title={item.label} icon={<DownOutlined style={{fontSize:'1vw'}}/>}>
            {item.subItems.map(subItem => (
              <Menu.Item key={subItem.key} style={{fontFamily:'pragmatica', fontSize:'0.9vw', fontWeight: 'lighter'}}>
    ...
    <div style={{backgroundImage: `url(${img})`, backgroundRepeat: 'no-repeat', backgroundSize: 'cover', height:'60vw'}}>
